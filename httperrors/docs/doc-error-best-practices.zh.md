...
> **约定**：`reason` 一律使用当前仍保留的 `httperrors.Reason*` 常量（如 `ReasonNotFound`、`ReasonInvalidArgument`），不硬编码字符串。  
...
func (r *CreateUserRequest) Validate() error {
    ...
    // 无错误时返回 nil（BadRequest 传入空切片时 Err() 返回 nil）
...
func (h *UserHandler) Create(w http.ResponseWriter, r *http.Request) {
    ...
    if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
        panic(httperrors.Error(http.StatusBadRequest, httperrors.ReasonInvalidArgument, "invalid request body"))
    }
...
> **要点**：`400 Bad Request` 在当前实现中默认对应 `ReasonInvalidArgument`，不要继续使用已移除的 `ReasonBadRequest`。`reason` 同时作为 i18n key，翻译自动填入 `localizedMessage`。
...
func (s *TransferService) Transfer(ctx context.Context, fromID, toID string, amount int64) error {
    ...
    if balance < amount {
        return httperrors.New(http.StatusBadRequest, httperrors.ReasonInvalidArgument, "insufficient balance for transfer").
            WithMetadata(map[string]string{
...
| ------------- | ----------- | ------------------------- | ---------------------------------------------------------- |
| 字段校验错误  | 400         | `ReasonInvalidArgument`   | `BadRequest(fvs)`                                          |
| 参数无效      | 400         | `ReasonInvalidArgument`   | `Error(400, ReasonInvalidArgument, msg)`                   |
...
| JSON 解析失败 | 400         | `ReasonInvalidArgument`   | `New(400, ReasonInvalidArgument, msg).WithCause(err)`      |

## 核心原则

1. **reason 用 `Reason*` 常量**：不硬编码字符串，如 `ReasonNotFound`、`ReasonInvalidArgument`
