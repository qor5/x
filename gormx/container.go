...
func OpenContainer(ctx context.Context, conf *ContainerConfig) (_ *Container, xerr error) {
	image := cmp.Or(conf.Image, "postgres:17.4-alpine3.21")
	username := cmp.Or(conf.Username, "postgres")
	password := cmp.Or(conf.Password, "postgres")
	databaseName := cmp.Or(conf.DatabaseName, "postgres")
	...
	return &Container{
		Container: container,
		DSN:       fmt.Sprintf("postgres://%s:%s@%s/%s?sslmode=disable", username, password, endpoint, databaseName),
	}, nil
}
