...
func HelloButton(ctx *ui.EventContext) (pr ui.PageResponse, err error) {
	...
	pr.Schema = h.Tag("div").Children(
		Button("Reload").Variant(ButtonVariantRaised).
			OnClick(ctx.Hub, "reload3", reload),
		Button("Reload").Variant(ButtonVariantUnelevated).
			OnClick(ctx.Hub, "reload4", reload),
		Button("Reload").Variant(ButtonVariantOutlined).
			OnClick(ctx.Hub, "reload5", reload),
		Button("Reload").Variant(ButtonVariantText).
