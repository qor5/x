...
// ChainUnaryServerInterceptors chains multiple unary server interceptors into one.
...
// ChainUnaryClientInterceptors chains multiple unary client interceptors into one.
// The first interceptor will be the outermost wrapper.
// The implementation mirrors gRPC's internal chaining strategy.
func ChainUnaryClientInterceptors(interceptors ...grpc.UnaryClientInterceptor) grpc.UnaryClientInterceptor {
	...
	case 1:
		return interceptors[0]
	default:
		return chainUnaryClientInterceptors(interceptors)
	}
}

// chainUnaryInterceptors is adapted from gRPC's internal server interceptor chaining.
...
// chainUnaryClientInterceptors is adapted from gRPC's internal client interceptor chaining.
func chainUnaryClientInterceptors(interceptors []grpc.UnaryClientInterceptor) grpc.UnaryClientInterceptor {
