...
var PageFuncAndEventFunc = Components(
	md.Markdown("`PageFunc` is used to build a web page, `EventFunc` is called when user interact with the page, For example button or link clicks."),
	ch.Code(samples.PageFuncAndEventFuncDefinition),
	md.Markdown("`ctx.Hub.RegisterEventFunc` is used to connect an `EventFunc` to a `PageFunc` and also provide an event name, to be used by `web.Bind` to attach to an html element."),
...
[gziphandler](https://github.com/nytimes/gziphandler) is an example`),
	...
	md.Markdown("But What the heck is `demoLayout` there? Well it's a `PageFunc` middleware. That takes an `PageFunc` as input, wrap it's `PageResponse` with layout html and return a new `PageFunc`. If you follow the code to write your own `PageFunc`, The button click might not work without this. Since there is no layout to import needed javascript to make this work. continue to next page to checkout how to add necessary javascript, css etc to make the demo work."),
)
